...
type PricesResponse = {
  ...
  recharges: {
    classic: PriceInfo | null;
    minute: PriceInfo | null;
    boost: PriceInfo | null;
  };
};
...
async function resolveSupabaseClient(primary: SupabaseClient): Promise<{
...
export default function ParametresAbonnementPage() {
  ...
  const [iapOffering, setIapOffering] = useState<PurchasesOffering | null>(
    null,
  );
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [{ data: infoRow }, { data: productsRow }, pricesRes] =
        await Promise.all([
          client
            .from("stripe_info")
            .select(
      ...
      const joinedUser = (infoRow as any)?.user as
        | UserProfile
        | null
        | undefined;
  ...
  const basicProductId = useMemo(
    () =>
      products.find((p) => p.name?.toLowerCase() === "basic")?.product_id ??
      null,
    [products],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const newClassic = (monthly.solde?.totalClassic ?? 0) + proratedClassic;
      const newLastminute =
  ...
  const openCustomerPortal = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${getApiBaseUrl()}/api/stripe/create-portal-session`,
        {
          method: "POST",
...
function RechargesSection({
  ...
  const totalCents =
    lineTotalCents("classic") +
    lineTotalCents("minute") +
    lineTotalCents("boost");
              ...
              return (
                ...
                >
                  <span
                    ...
                    aria-hidden="true"
                  />
                  <span className="flex-1 text-pine-500">
                    {def.productLabel}
                  </span>
...
function CounterTile({ label, counter }: { label: string; counter: number }) {
  return (
...
function IapRechargesCart({
  ...
  const totalQty = quantities.classic + quantities.minute + quantities.boost;
  ...
  return (
    <div className="divide-y divide-grey-100 border-y border-grey-100">
      {ROWS.map((row) => {
        ...
        const qty = quantities[row.key];
        return (
          <div key={row.key} className="flex items-center gap-3 py-3 text-sm">
            <span
              className={cn(
                "h-2 w-2 shrink-0 rounded-full",
                DOT_COLOR[row.key],
  ...
  return (
    <section className="mb-6 rounded-2xl bg-white p-6 shadow-sm">
      ...
      <p className="text-sm text-grey-600">
        Mettez à jour votre carte, consultez vos reçus et téléchargez vos
