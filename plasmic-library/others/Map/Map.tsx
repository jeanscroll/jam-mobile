...
const Mapbox: React.FC<MapboxProps> = ({
  ...
  // Token public Mapbox. Les variables NEXT_PUBLIC_* sont inlinées au build :
  // si l'environnement de build mobile (Appflow/CI) ne la définit pas, le
  // bundle natif contient `undefined` et la carte ne s'initialise jamais.
  // On retombe alors sur ce token public commité (pattern déjà utilisé pour
  // les clés Stripe/Google dans capacitor.config.ts).
  // ⚠️ FIXME: coller ici le token public Mapbox ("pk....") et le restreindre
  // par URLs autorisées dans le dashboard Mapbox.
  const MAPBOX_PUBLIC_TOKEN_FALLBACK = "";
  const accessToken =
    process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN || MAPBOX_PUBLIC_TOKEN_FALLBACK;
  ...
  useEffect(() => {
    ...
    // Sans token, mapbox-gl ne s'initialise pas : on remonte une erreur visible
    // au lieu d'un échec silencieux (le bug "aucune carte affichée").
    if (!accessToken) {
      console.error(
        "[Map] Token Mapbox manquant : définir NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN " +
          "au build, ou MAPBOX_PUBLIC_TOKEN_FALLBACK dans Map.tsx."
      );
      setMapError("Carte indisponible (configuration manquante).");
      return;
    }
    ...
    try {
      ...
      console.error("[Map] Échec d'initialisation Mapbox:", err);
      setMapError("Carte indisponible.");
      return;
    }
    ...
    map.on("error", (e) => {
      // Token invalide, style introuvable, tuiles bloquées (CORS/CSP webview)…
      console.error("[Map] Erreur Mapbox:", e?.error?.message || e);
    });
    ...
    <>
      ...
      <div
        ...
        style={{
          width: "100%",
          height: "100%",
          // Garde-fou : mapbox-gl exige une hauteur résolue. Si le conteneur
          // parent (page Plasmic) n'impose pas de hauteur, height:100% vaut 0
          // et la carte reste invisible. minHeight évite ce cas.
          minHeight: 240,
          borderRadius: "16px",
          position: "relative",
        }}
