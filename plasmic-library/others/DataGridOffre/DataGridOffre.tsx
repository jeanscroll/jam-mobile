...
export const DataGridOffre: React.FC<DataGridOffreProps> = ({
  ...
  // Filet de sécurité : si le chargement (query Supabase côté client en WebView)
  // ne se résout jamais, on évite un "Chargement..." infini en affichant un
  // message de repli après un délai. Cf. bug /annonces "page blanche Chargement".
  useEffect(() => {
    if (!isLoading) {
    ...
    if (loadTimedOut) {
      return (
        <div className={styles.error}>
          ...
          <p>
            Impossible de charger les données. Vérifiez votre connexion puis
            réessayez.
          </p>
        </div>
      );
  ...
  if (offres.length === 0) {
    return <div className={styles.empty}>{emptyStateMessage}</div>;
  }
